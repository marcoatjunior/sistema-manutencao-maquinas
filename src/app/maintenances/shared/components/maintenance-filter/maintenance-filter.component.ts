...
export class MaintenanceFilterComponent implements OnInit, OnDestroy {
    ...
    private startDateChanges() {
        this.formGroup.get('start_date').valueChanges.pipe(untilDestroyed(this)).subscribe(value => {
    ...
    private endDateChanges() {
        this.formGroup.get('end_date').valueChanges.pipe(untilDestroyed(this)).subscribe(value => {
